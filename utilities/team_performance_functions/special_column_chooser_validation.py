...
special_team_coll_list = [
    'Column Department', 'Column Team Member', 'Column Completed', 'Column Pending', 'Column Overdue',
    'Column Today', 'Column Next 6 Days', 'Column 8 To 30 Days', 'Column Approval Pending', 'Column Rejected'
]


def special_column_chooser_flow(driver, wait):
    final_status = True  
    try:
    ...
    try:
        ...
        # ==============================
        with allure.step("Validate All Special Team Performance Columns Visible"):
            headers = wait.until(EC.presence_of_all_elements_located((By.XPATH, "//tr[contains(@class,'dx-header-row')]//td")))
            header_texts = []
            ...
            for h in headers:
                try:
                    ...
                    if txt:
                        ...
                        print(f"Header Found: {txt}")

                except Exception as e:
                    print(f"Unable to read header: {e}")

            print(f"📊 All Headers: {header_texts}")
            allure.attach(str(header_texts), name="All Headers", attachment_type=allure.attachment_type.TEXT)

            required_headers = ["Department", "Team Member", "Completed", "Complied", "Not Complied", "Pending", "Overdue", "Today",
                "Next 6 Days", "8 To 30 Days", "Approval Pending", "Rejected"
            ]

            missing = [col for col in required_headers if col not in header_texts]

            if missing:
                pytest.fail(f"❌ Missing Headers: {missing}")
        ...
        # ==============================
        with allure.step("Validate Partial Selection for Special Team Performance"):
            headers = wait.until(
                EC.presence_of_all_elements_located(
                    (By.XPATH, "//tr[contains(@class,'dx-header-row')]//td")
                )
            )
            ...
            print(f"📊 Partial Headers: {visible_headers}")
            allure.attach(str(visible_headers), name="Partial Headers", attachment_type=allure.attachment_type.TEXT)

            if "Team Member" not in visible_headers:
        ...
        # ✅ STEP 3: VALIDATE ALL COLUMNS
        # ==============================
        with allure.step("Final Validation - All Special Team Columns"):
            headers = wait.until(EC.presence_of_all_elements_located((By.XPATH, "//tr[contains(@class,'dx-header-row')]//td")))
            ...
            print(f"📊 Final Headers: {header_texts}")
            allure.attach(str(header_texts), name="Final Headers", attachment_type=allure.attachment_type.TEXT)

            required_headers = [
                "Department", "Team Member", "Completed", "Complied", "Not Complied", "Pending", "Overdue",
                "Today", "Next 6 Days", "8 To 30 Days", "Approval Pending", "Rejected"
            ]

            missing = [col for col in required_headers if col not in header_texts]

            if missing:
                pytest.fail(f"❌ Missing Final Headers: {missing}")
