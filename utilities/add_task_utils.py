...
def add_task_check(driver, task_name, start_date, due_date, frequency, repeat_if_due_date_is_on_holiday, end_frequency_date, weekday_name, repeat_day_and_month, end_time, internal_deadline, assign_to, approver, cc, risk_rating, license_name, description, attach_file_name, impact_details, impact_file_name, circular_search, test_type, task_type=None, refresh=False, login_required=True, direct_task_creation = False, module="default"):
    ...
    if task_type == 'mandatory':
        ...
        with allure.step(f"Click Submit button"):
            print()
            print(f"Submitting Task:")
            submit_task_success = submit_task(driver=driver, submit_button=submit_button, toast_msg=toast_msg, wait=wait)
            if submit_task_success:
                msg = "Task submitted successfully."
                print(msg)
                allure.attach(msg, name="Submit Task Status", attachment_type=allure.attachment_type.TEXT)
...
def get_test_case_list(module=None):
    try:
        ...
        for _, row in test_case_details.iterrows():
            ...
            # raw_level = row.get("level_of_testing")
            ...
            test_case_list.append(
                pytest.param(
                    ...
                    format_date_if_valid(row['end_frequency_date(YYYY-MM-DD)']),
