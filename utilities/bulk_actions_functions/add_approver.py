...
def approver_bulk_action(driver, wait, dropdown_selection_val_data, task_name='Internal Task', text_case_id=None):
    ...
    with allure.step("Select 'Company / Project, Approver, CC, Creator' from Column Chooser"):    
        try:
            ...
            time.sleep(0.5)
            wait.until(EC.element_to_be_clickable((By.XPATH, column_chooser_cc)))
            cc_option.click()
    ...
    with allure.step("Open CC column filter and search for 'Assign'"):
        cc_filter_btn = wait_less.until(EC.presence_of_element_located((By.XPATH, "//td[@role='columnheader'][.//text()[normalize-space()='CC']]//span[contains(@class,'dx-header-filter')]")))
        cc_filter_btn.click()
    ...
    with allure.step("Open Bulk Action and select 'Add Approver'"):
        ...
        approver_option = wait.until(EC.element_to_be_clickable((By.XPATH, "//div[contains(@class,'dx-list-item-content') and text()='Add Approver']")))
