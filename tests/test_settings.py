...
from utilities.settings_utils import get_test_case_list, settings_check
from utilities.driver_setup import setup
from utilities.screen_recorder import ScreenRecorder


at_test_case_list = get_test_case_list(module='settings')
@allure.suite("Settings Test Suite")
@allure.sub_suite("Settings Validation")
...
def test_settings_flow(setup, test_case_id, module_name, test_case_description, test_type, task_details, test_case_execution):
    # test_tp_flow allure.dynamic.title(f"Dashboard Validation")
    allure.dynamic.title(f"{test_case_id}_{module_name}")
    allure.dynamic.description(f'{test_case_description}')
    driver = setup
    test_name = f"settings_validation"
    ...
    time.sleep(2)
    test_failed = False
    with allure.step(f"Settings Flow"):
        try:
            success = settings_check(driver, module_name=module_name, test_case_id=test_case_id, task_details=task_details)
            # if success and test_type == 'positive':
            if test_type == 'positive':
                # assert True
                   assert success, "Settings failed with valid scenario"
            elif test_type == 'negative':
                assert not success, "Settings succeeded unexpectedly for an invalid scenario"
