...
from utilities.notifications_utils import get_test_case_list, notifications_check
from utilities.driver_setup import setup
from utilities.screen_recorder import ScreenRecorder

at_test_case_list = get_test_case_list(module='notifications')
@allure.suite("Notifications Test Suite")
@allure.sub_suite("Notifications Validation")
...
def test_notifications_flow(setup, test_case_id, module_name, test_case_description, test_type, test_case_execution):
    ...
    test_name = f"notifications_validation"
    ...
    with allure.step(f"Notifications Flow"):
        try:
            ...
            if test_type == 'positive':
                # assert True
                   assert success, "Notifications Dashboard failed with valid scenario"
            elif test_type == 'negative':
                assert not success, "Notifications Dashboard succeeded with invalid scenario"
