...
from utilities.project_utils import get_test_case_list, create_project_check
from utilities.driver_setup import setup
from utilities.screen_recorder import ScreenRecorder

at_test_case_list = get_test_case_list(module='project')
@allure.suite("Project Test Suite")
@allure.sub_suite("Project Validation")
...
def test_project_flow(setup, test_case_id, module_name, test_case_description, test_type, task_details, test_case_execution):
    ...
    driver = setup
    test_name = f"project_validation"
    ...
    with allure.step(f"Project Flow"):
        try:
            ...
            success = create_project_check(driver, module_name=module_name, test_case_id=test_case_id, task_details=task_details)
            # if success and test_type == 'positive':
            if test_type == 'positive':
                # assert True
                   assert success, "Project Dashboard failed with valid scenario"
