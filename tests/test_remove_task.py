...
from utilities.remove_task_utils import get_test_case_list, remove_task_check
...
def test_delete_task_flow(setup, test_case_id, module_name, test_case_description, test_type, test_case_execution):
    ...
    test_name = f"delete_task_validation"
    ...
    time.sleep(2)
    test_failed = False
    with allure.step(f"Trying to validate UI of remove task"):
        try:
            success = remove_task_check(driver, module_name=module_name)
