...
from utilities.calendar_utils import get_test_case_list, calendar_check
...
def test_project_flow(setup, test_case_id, module_name, test_case_description, test_type, task_details, test_case_execution):
    ...
    driver = setup
    test_name = f"Calendar_validation"
    ...
    with allure.step(f"Calendar Flow"):
        try:
            print(f'module_name: {module_name}')
            success = calendar_check(driver, module_name=module_name, test_case_id=test_case_id, task_details=task_details)
            # if success and test_type == 'positive':
            if test_type == 'positive':
                # assert True
                   assert success, "Calendar Dashboard failed with valid scenario"
            elif test_type == 'negative':
                assert not success, "Calendar Dashboard succeeded with invalid scenario"
