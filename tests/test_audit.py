...
from utilities.audit_utils import get_test_case_list, audit_check
from utilities.driver_setup import setup
from utilities.screen_recorder import ScreenRecorder

at_test_case_list = get_test_case_list(module='audit')

@allure.suite("Audit Test Suite")
@allure.sub_suite("Audit Validation")
...
def test_audit_flow(setup, test_case_id, module_name, test_case_description, test_type, task_details, test_case_execution):
    ...
    with allure.step(f"Audit Flow"):
        try:
            
            success = audit_check(driver, module_name=module_name, task_details=task_details)
            # if success and test_type == 'positive':
            if test_type == 'positive':
                # assert True
                assert success, "Audit failed with valid scenario"
            elif test_type == 'negative':
                assert not success, "Audit succeeded with invalid scenario"
