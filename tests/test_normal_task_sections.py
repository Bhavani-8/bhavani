...
from utilities.task_sections_utils import get_test_case_list, task_sections_check
from utilities.driver_setup import setup
from utilities.screen_recorder import ScreenRecorder

at_test_case_list = get_test_case_list(module='normal_task_sections')
@allure.suite("Normal Task Sections Test Suite")
@allure.sub_suite("Normal Task Sections Validation")
@pytest.mark.parametrize("test_case_id,module_name,test_case_description,test_type,test_case_execution", at_test_case_list)
def test_normal_task_sections_flow(setup, test_case_id, module_name, test_case_description, test_type, test_case_execution):
    allure.dynamic.title(f"{test_case_id}_{module_name}")
    allure.dynamic.description(f'{test_case_description}')
    driver = setup
    test_name = f"normal_task_sections_validation"
    ...
    time.sleep(2)
    test_failed = False
    with allure.step(f"Normal Task Sections Flow"):
        try:
            success = task_sections_check(driver, module_name=module_name, test_case_id=test_case_id)
            # if success and test_type == 'positive':
            if test_type == 'positive':
                # assert True
                   assert success, "Normal Task Sections Dashboard failed with valid scenario"
            elif test_type == 'negative':
                assert not success, "Normal Task Sections Dashboard succeeded with invalid scenario"
